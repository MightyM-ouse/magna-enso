...
```mermaid
flowchart LR
  UI["React ten-tab shell - App.tsx - MAG-EXP"] --> REST["REST client - apiClient.ts - MAG-INT"]
  UI --> WS["Workflow/runtime WebSockets - workflowSocket.ts - MAG-OBS"]
  REST --> API["FastAPI app - main.py - MAG-INT"]
  WS --> API
  API --> ROUTER["CSF to BRS routing - routes_local_model.py + intent_router.py - MAG-COG"]
  API --> POLICY["Risk classify + authorize + approvals - MAG-GOV"]
  API --> RUNTIME["Event bus to workflow to orchestration - MAG-ORC"]
  RUNTIME --> DB["SQLite SQLModel - models.py repositories.py - MAG-MEM"]
  POLICY --> DB
  RUNTIME --> OBS["Replay + runtime observability + WS dispatcher - MAG-OBS"]
  ROUTER --> OLLAMA["Local Ollama adapter - MAG-AGT"]
  ROUTER --> OPENAI["Env-gated OpenAI review - MAG-AGT"]
  ROUTER --> WEBS["Web-search agent - mock/default-disabled - MAG-TOL"]
```
...
```mermaid
flowchart LR
  REQ["Capability request - harness - MAG-TOL"] --> GATE["CapabilityGate.authorize - single chokepoint - MAG-GOV"]
  GATE --> EVAL["PolicyEvaluator.evaluate - default-deny - MAG-GOV"]
  EVAL --> SCHEMA["load_policy_records - strict JSON - MAG-GOV"]
  GATE --> COORD["ApprovalCoordinator.consume - MAG-GOV"]
  COORD --> FP["build_fingerprint - SHA-256 canonical - MAG-SEC"]
  GATE --> AUDIT["SecureAuditSink - 0600 hash-chain JSONL - MAG-SEC MAG-OBS"]
  COORD -. test-only provider under tests/ .-> NULL["Null/Deny provider default - MAG-GOV"]
```
