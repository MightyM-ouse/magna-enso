...
## AI navigation index
- `memory_model` → §1 (MAG-MEM)
- `persistence_evidence` → §2 (DIAG-12)
- `audit_durability` → §3 (MAG-SEC MAG-OBS)
- `replay_recovery` → §4 (DIAG-20)

## 1. Memory model (MAG-MEM) — `Status: traceability now; governed working memory PLANNED`
...
```mermaid
flowchart LR
  RUN["Runtime - MAG-ORC"] --> EVT["Durable events - causal/correlation IDs - MAG-OBS"]
  EVT --> DB["Persistence - SQLite SQLModel today - MAG-MEM"]
  RUN --> APPR["Approval records - lifecycle - MAG-GOV"]
  APPR --> DB
  GATE["Capability gate - MAG-GOV"] --> AUD["Secure audit - 0600 hash-chain JSONL - MAG-SEC"]
  AUD --> RECON["Replay/reconstruct state - MAG-OBS"]
  DB --> RECON
```
...
```mermaid
flowchart TD
  RESTART["Process restart - MAG-OBS"] --> RESET["Resting state = default-deny - MAG-GOV"]
  HOLDS{"In-flight approval HOLDs?"} -->|yes| FRESH["No auto-allow; fresh decision required; production absent => DENY"]
  HOLDS -->|no| RESET
  LOG["Append-only audit log = recovery source of truth - MAG-OBS"] --> REBUILD["Reconstruct decisions/approvals"]
  REBUILD --> RESET
  CLOCK["Clock rollback/uncertainty"] --> INVALID["Invalidate pending approvals => DENY - MAG-GOV"]
```

Recovery facts (`05`): pending approvals memory-only, discarded on restart; expiry uses monotonic time
(resets on restart); clock rollback invalidates approvals ⇒ DENY; wall-clock is **evidence only**, never expiry
