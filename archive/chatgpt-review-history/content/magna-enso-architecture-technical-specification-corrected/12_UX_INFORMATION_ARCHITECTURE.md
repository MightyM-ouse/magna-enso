...
## AI navigation index
- `frozen_shell` → §1 (MAG-EXP-001)
- `ten_tab_ia` → §2 (DIAG-17)
- `journeys` → §3 (MAG-EXP-001)
- `a11y_responsive_perf` → §4 (MAG-UX)

## 1. The frozen canonical shell (MAG-EXP-001) — **FROZEN, do not alter**
...
```mermaid
flowchart TB
  SHELL["Magna shell + Presence - MAG-EXP-001 - verified"]
  SHELL --> CMD["Command - intent entry, active task, history"]
  SHELL --> IDN["Identity - truthful self-model - MAG-IDN-001"]
  SHELL --> AGT["Agents - workers/models - MAG-AGT-001"]
  SHELL --> MEM["Memory - traceability/recall - MAG-MEM-001"]
  SHELL --> EXP["Explorer - artifacts/files"]
  SHELL --> COG["Cognition - routing visibility - MAG-COG-001"]
  SHELL --> COS["Cosmos - ratified chronicle - MAG-COS-001"]
  SHELL --> HLP["Help"]
  SHELL --> SET["Settings - consent, providers"]
  SHELL --> SYS["System - status, diagnostics - MAG-OBS-001"]
```
