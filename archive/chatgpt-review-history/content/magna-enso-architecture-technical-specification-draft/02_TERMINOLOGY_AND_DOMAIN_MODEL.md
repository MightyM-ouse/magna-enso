...
Component-level IDs append a two-digit suffix (e.g. `MAG-COG-01`). The authoritative component list is
...
## 4. Status model (used on every component/requirement/diagram node)
...
```mermaid
flowchart TD
  VINAY["Vinay - final authority - MAG-GOV"]
  HLX["HELIX genome - read-only - MAG-HLX"]
  MAGNA["Magna runtime - MAG-PRG"]
  IDN["Identity - current self-model - MAG-IDN"]
  COS["Cosmos - ratified chronicle - MAG-COS"]
  PRES["Presence - UI projection - MAG-EXP"]
