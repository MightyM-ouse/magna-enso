...
## AI navigation index
- `memory` → §2 (MAG-MEM-001) · `persistence` → §3 · `governed_memory` → §4
...
Schemas (PROPOSED for the target architecture within the existing magna-enso repository): events, approvals, audit records, draft-state. Compose MCC durability with
