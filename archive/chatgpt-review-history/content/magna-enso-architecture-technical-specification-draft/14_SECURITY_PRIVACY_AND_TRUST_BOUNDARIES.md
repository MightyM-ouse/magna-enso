...
## AI navigation index
- `principles` → §1 (MAG-SEC)
- `zones` → §2 (DIAG-19)
- `secrets_privacy` → §3 (MAG-SEC)
- `audit_security` → §4 (MAG-SEC)
...
```mermaid
flowchart TB
  subgraph TRUSTED["Trusted zone - local owner - MAG-SEC"]
    VINAY["Vinay - authenticated owner"]
    GATE["Capability gate + approvals - MAG-GOV"]
    CORE["Runtime core + persistence + audit - MAG-ORC MAG-MEM MAG-SEC"]
  end
  subgraph SEMI["Semi-trusted - workers/models - MAG-AGT"]
  ...
  CORE -. read-only facts .-> VERIFIER["Independent verifier - MAG-TRC"]
```
