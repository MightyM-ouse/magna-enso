...
current_vs_target: Routing pattern verified-current (MCC); target routing (within the existing magna-enso repository) PLANNED
date: 2026-06-21
...
## AI navigation index
- `interpret` → §2 (MAG-INT-001) · `routing` → §3 (MAG-COG-001) · `reflex` → §4 (MAG-GOV-001)
...
## 4. Reflex vs cognition (MAG-GOV-001)
...
Status: pattern `IMPLEMENTED_VALIDATED` (MCC); target (within the existing magna-enso repository) `PLANNED`. Open: OD-07.1 reuse vs reimplement router;
